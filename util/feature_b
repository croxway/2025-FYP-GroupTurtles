...
image_files = [
    f for f in glob(os.path.join(image_folder, "*.png")) + 
...
def process_image(image_path):
    ...
    if image is None or mask is None:
        print("  Error reading image/mask.")
        return None
...
with open(output_csv_raw, mode='w', newline='') as file:
    ...
    results = Parallel(n_jobs=-1)(delayed(process_image)(img_path) for img_path in image_files)
...
print("Scaled and rounded features saved to:", output_csv_scaled)
